...
<link rel="stylesheet" href="oneway-1.css">
</head>
<body>
...
<!--*******************Redirect to the RoundTrip Page************************************************************************************************************************* -->
	<% 
		if(request.getParameter("rounddate")!= null)
		{
			session.setAttribute("origin", request.getParameter("origin"));
			session.setAttribute("destination", request.getParameter("destination"));
			session.setAttribute("onedate", request.getParameter("onedate"));
			session.setAttribute("twodate", request.getParameter("rounddate"));
			response.sendRedirect("RoundTrip1.jsp");
		}
   	 ...
   	 <%
   		while(rows.next())
   		{
   			...
   			<tr>
   			  ...
   			  <td>
   			 	<div align="center" class="tableview" ><a href="OneWay1.jsp?idd=<%=rows.getString("rid")%>"style = "font-family: 'Arial'">Stops</a></div>				  	
...
<!--*******************Submitting Table************************************************************************************************************************* --> 
		...
		<br>
		<input type="submit" class="submit-btn" value="Submit" name="submit0">
...
<!--*******************Stops Popup************************************************************************************************************************* --> 
	...
	%>
			...
			<%
						...
						String sql1 = "select  s.tid,st.name, sa.sarrivaltime, sa.sdeparturetime from RailwayDB.stopsat as sa, RailwayDB.Schedule as s, RailwayDB.Station as st where sa.rid = s.rid and sa.sid = st.sid and s.rid = "+request.getParameter("idd")+";";
